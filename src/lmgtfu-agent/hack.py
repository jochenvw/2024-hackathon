...
# Load the agents
from agents import prompt_engineer, researcher, research_manager, executor, user_proxy, result_critic, azure_policy_expert
...
register_function(scrape_website, caller=azure_policy_expert.agent, executor=executor.agent, name="web_scraping", description="A tool to get the contents from of a specific URL of a website", )
register_function(google_search, caller=azure_policy_expert.agent, executor=executor.agent, name="google_search", description="A tool to search the internet or web for a specific topic", )


# Create group chat
groupchat = autogen.GroupChat(agents=[user_proxy.agent, prompt_engineer.agent, researcher.agent, research_manager.agent, executor.agent, result_critic.agent, azure_policy_expert.agent], messages=[], max_round=100)
group_chat_manager = autogen.GroupChatManager(groupchat=groupchat, llm_config={"config_list": config_list_gpt4})


message = """
What of the following Azure services can be backed up using the Azure Backup Service: Azure Compute? Return the results in a markdown table. Check if it can be enforced by Azure Policy. Provide links to the official documentation as well as quotes from the documentation that support your answer.
"""
